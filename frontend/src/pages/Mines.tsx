...
import { Settings, Volume2, HelpCircle, Gem, Bomb } from 'lucide-react';
import { useMinesGame } from '../hooks/useMinesGame';
import { useChecko } from '../hooks/useChecko';

export const Mines = () => {
    // Game Hooks
    const { gameState, balance, loading, startGame, revealTile, cashOut, refreshState } = useMinesGame();
    const { isConnected, connect } = useChecko();

    // Local UI State
    const [betAmount, setBetAmount] = useState<number>(0);
    const [minesCount, setMinesCount] = useState<number>(3); // Default 3
    ...
    const isGameWon = gameState?.result === 'Won' || gameState?.result === 'CashedOut';
